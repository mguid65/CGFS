...
#ifndef CGFS_LOGGER_HPP
#define CGFS_LOGGER_HPP
...
[[nodiscard]] std::string_view log_level_string() {
  ...
  if constexpr (Level == LogLevel::info) { return {"Info"}; }
  if constexpr (Level == LogLevel::warning) { return {"Warning"}; }
  if constexpr (Level == LogLevel::error) { return {"Error"}; }
  if constexpr (Level == LogLevel::critical) { return {"Critical"}; }
}
...
struct DefaultFormatter {
  ...
  [[nodiscard]] std::string format_log(Args&&... args) const {
    if constexpr (WithSrcLoc) {
      return fmt::format("{datetime}|{level}|{src_loc}|{msg}", std::forward<Args>(args)...);
    } else {
      return fmt::format("{datetime}|{level}||{msg}", std::forward<Args>(args)...);
    }
  }
};

struct StdoutLogSink {
  static void log(std::string_view msg) { std::cout << msg << std::endl; }
};
...
struct LogSink {
  ...
  public:
    ...
    void log(std::string_view msg) override { m_sink.log(msg); }
    void add_filter() {}
    virtual ~LogSinkModel() = default;
  };

  template <typename Concrete>
  LogSink(Concrete&& sink)
      : m_concept{std::make_shared<LogSinkModel<Concrete>>(std::forward<Concrete>(sink))} {}
...
struct Logger {
  struct LogCtx {
    LogCtx(Logger& logger, std::source_location src_loc) : m_logger{logger}, m_src_loc{src_loc} {}
  ...
  template <typename... Args>
  void debug(std::source_location src_loc, fmt::format_string<Args...> fmt_str, Args&&... args) {
    ...
    log<LogLevel::debug>(m_formatter.template format_log<EnableSrcLocation>(
        ...
        fmt::arg("src_loc", fmt::format("{}:{}:{}", src_loc.file_name(), src_loc.function_name(),
                                        src_loc.line())),
        fmt::arg("msg", fmt::format(fmt_str, std::forward<Args>(args)...))));
  }
  template <typename... Args>
  void info(std::source_location src_loc, fmt::format_string<Args...> fmt_str, Args&&... args) {
    const auto time = std::chrono::system_clock::now();
  ...
  template <typename... Args>
  void warning(std::source_location src_loc, fmt::format_string<Args...> fmt_str, Args&&... args) {
    const auto time = std::chrono::system_clock::now();
    log<LogLevel::warning>(m_formatter.template format_log<EnableSrcLocation>(
        fmt::arg("datetime", fmt::format("{:%Y%m%d-%X}", time)),
  ...
  template <typename... Args>
  void error(std::source_location src_loc, fmt::format_string<Args...> fmt_str, Args&&... args) {
    const auto time = std::chrono::system_clock::now();
  ...
  template <typename... Args>
  void critical(std::source_location src_loc, fmt::format_string<Args...> fmt_str, Args&&... args) {
    const auto time = std::chrono::system_clock::now();
    log<LogLevel::critical>(m_formatter.template format_log<EnableSrcLocation>(
        fmt::arg("datetime", fmt::format("{:%Y%m%d-%X}", time)),
  ...
  LogCtx with_ctx(std::source_location src_loc = std::source_location::current()) {
...
private:
  ...
  std::vector<std::pair<std::string, LogSink>> m_sinks{{"default", StdoutLogSink{}}};
//  FilteringStdoutLogSink<decltype([](std::string_view msg) -> std::string {
//    //         if (msg.empty()) { return {""}; }
...
//  })>{}}
  ...
  void log(std::string_view msg) {
    ...
    for (auto& sink : m_sinks) { sink.second.log(msg); }
  }
};

template <typename Formatter = DefaultFormatter, bool EnableSrcLocation = true>
Logger<Formatter, EnableSrcLocation>& get_logger() {
  static Logger<Formatter, EnableSrcLocation> logger{};
  return logger;
}

#endif  // CGFS_LOGGER_HPP
